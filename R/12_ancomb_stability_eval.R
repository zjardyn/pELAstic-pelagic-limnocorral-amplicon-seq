...
stability_data_16S <- readRDS("out/ancombc_stability_16S_9_ws_trend.rds")
stability_data_18S <- readRDS("out/ancombc_stability_18S_9_ws_trend.rds")
...
# Create mappings for display labels
...
ggsave("figures/trend_analysis_heatmap_stable_taxa_combined.png", p_combined, 
       width = 10, height = 12, dpi = 300, scale = 0.8)
...
p_heatmap_16S_transposed <- ggplot(heatmap_data_16S_transposed, aes(x = taxon_display, y = plastic_level, fill = lfc)) +
  ...
  theme(
    axis.text.x = element_text(
      color = ifelse(taxon_display_passed_ss_16S[levels(heatmap_data_16S_transposed$taxon_display)], "#000000", "#8a8a8a"),
      size = 10,
      angle = 45,
      hjust = 1,
      vjust = 1
