...
# Set Date to 10 for microscope slide samples
phy_16S <- phy_16S %>% ps_mutate(Date = ifelse(Location == "MS", 10, Date))
phy_18S <- phy_18S %>% ps_mutate(Date = ifelse(Location == "MS", 10, Date))
phy_16S <- phy_16S %>% ps_mutate(Date = factor(Date, levels = c(3, 6, 9, 10)))
